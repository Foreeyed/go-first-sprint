...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	allSpentCalories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration), 2)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return allSpentCalories
}
